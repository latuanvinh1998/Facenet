...
def model_evaluate(model, pair_path, lfw_dir, batch_size=8, nrof_fold=128 ,embedding_size=128):
    ...
    evaluate_data = tf.data.Dataset.from_tensor_slices((image_paths_array, labels_array))
    evaluate_data = evaluate_data.repeat(2).shuffle(5000).batch(batch_size).prefetch(1)
    
    nrof_batches = int(np.ceil(nrof_images / batch_size))
    evaluate_emb_array = np.zeros((nrof_images, 128))

    for (x_batch, y_batch) in evaluate_data.take(nrof_batches):
        evaluate_images = []
        for filename in tf.unstack(x_batch):
            ...
            evaluate_images.append(image)
        embedding = model(evaluate_images, True)
        evaluate_emb_array[y_batch,:] = embedding
    tpr, fpr, acc = evaluate(evaluate_emb_array, y_true, nrof_folds=nrof_fold)
